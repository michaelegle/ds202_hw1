...
A job that is related to data science is a director of research for a professional sports team. Popularized by the ideas in the book *Moneyball* about the Oakland Athletics, there's been a slow but steady growth of data analysts for helping sports teams. Now, the National Football League even has its own [annual data competition](https://operations.nfl.com/the-game/big-data-bowl/) to showcase professional and amateur data scientists for teams looking to hire. I've even played around with sports data on my own, specifically with **win probability** charts: ![](/Users/michaelegle/Dropbox/GB_CHI_W1.jpeg)
...
6. Configure Git by modifying and entering the following commands into the terminal
    ...
    ```
    git config user.name "Michael Egle"
    git config user.email "mjegle@iastate.edu"
    ```
